...
/// <summary>
/// Thread-safe in-memory store of the latest per-transfer telemetry snapshot per job. Registered as a
...
public sealed class DownloadTelemetryStore : IDownloadTelemetryStore
{
    ...
    private sealed record Entry(DateTime At, IReadOnlyList<DownloadTransferTelemetry> Transfers);

    public void Update(int jobId, IReadOnlyList<DownloadTransferTelemetry> transfers)
    {
        var now = DateTime.UtcNow;
        _snapshots[jobId] = new Entry(now, transfers);
    ...
    public IReadOnlyList<DownloadTransferTelemetry> Get(int jobId) =>
        _snapshots.TryGetValue(jobId, out var e) && DateTime.UtcNow - e.At <= StaleAfter
            ? e.Transfers
            : Array.Empty<DownloadTransferTelemetry>();
