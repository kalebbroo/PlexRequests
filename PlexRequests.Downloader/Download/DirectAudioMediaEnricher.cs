...
/// lookup. It deliberately supports JPEG/PNG from known YouTube image hosts and never follows redirects.</summary>
    ...
    ILogger<DirectAudioMediaEnricher> logger) : IDirectAudioMediaEnricher
{
    ...
    internal static bool TryValidateArtworkUrl(string? value, out Uri uri)
    {
        ...
        if (!host.Equals("i.ytimg.com", StringComparison.OrdinalIgnoreCase)
            ...
            && !host.EndsWith(".googleusercontent.com", StringComparison.OrdinalIgnoreCase)) return false;
        uri = parsed;
        return true;
    }
