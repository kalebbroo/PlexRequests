    ...
    ILogger<MediaMetadataScanWorker> logger) : BackgroundService
{
    ...
    internal async Task<MediaMetadataScanPass> RunOnceAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            if (!volume.IsReady)
                return await FailAsync(task, $"Library filesystem is unavailable: {volume.Error}", ct);
            if (!File.Exists(path))
                return await FailAsync(task, "The audited library file is no longer present", ct);
            ...
            if (!tracks.HasVideo || video is null || VideoCodecPolicy.Normalize(video.Codec) is null)
                return await FailAsync(task, "MediaInfo did not return a readable video codec", ct);
            ...
            var detail = $"Detected {VideoCodecPolicy.Display(video.Codec)}"
                         + (quality != PlexRequestsHosted.Shared.Enums.Quality.Any
                             ? $" at {quality.Label()}"
                             : string.Empty);
            var reported = await api.ReportMediaMetadataScanAsync(new MediaMetadataScanReportDto
            {
        ...
        {
            return await FailAsync(task, $"Retired library path skipped: {ex.Message}", ct);
        }
        ...
        catch (Exception ex)
        {
            ...
            return await FailAsync(task, ex.Message, CancellationToken.None);
        }
    }
    ...
    {
        await api.ReportMediaMetadataScanAsync(new MediaMetadataScanReportDto
        {
            ImportedFileId = task.ImportedFileId,
            WorkerId = _workerId,
            Succeeded = false,
