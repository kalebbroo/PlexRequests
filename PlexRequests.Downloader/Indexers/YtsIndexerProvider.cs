...
/// </summary>
public class YtsIndexerProvider(HttpClient http, ILogger<YtsIndexerProvider> logger) : IIndexerProvider
{
    ...
    public async Task<IReadOnlyList<ReleaseCandidate>> SearchAsync(FulfillmentJobDto job, CancellationToken ct)
    {
        ...
        var resp = await _http.GetFromJsonAsync<YtsResponse>(
            $"/api/v2/list_movies.json?query_term={Uri.EscapeDataString(term)}&limit=50", IndexerJson.Options, ct);
