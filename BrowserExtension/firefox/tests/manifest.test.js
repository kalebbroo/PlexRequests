...
test("manifest is Firefox MV3 and never requests cookie or browsing-history access", () => {
  ...
  assert.ok(manifest.background.scripts.includes("telemetry.js"));
  assert.ok(manifest.background.scripts.includes("capture-queue.js"));
  assert.ok(manifest.background.scripts.includes("background.js"));
  assert.ok(manifest.content_scripts.every(script => script.js.includes("capture-queue.js")));
